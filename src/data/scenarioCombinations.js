export default { 
	scenarioCombinations : 
		{ 
			scenarioOptions : [ 
				{ 
					"id": 0, 
					"name": "CNN", 
					"nameNoOptions": "CNN", 
					"short_description": "CNN", 
					"ultra_short_description": "CNN", 
					"desc": "long and eloquent scenario description", 
					"opt0": false, 
					"opt1": false, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 1, 
					"name": "CNN_copy", 
					"nameNoOptions": "CNN_copy", 
					"short_description": "CNN", 
					"ultra_short_description": "CNN", 
					"desc": "long and eloquent scenario description", 
					"opt0": false, 
					"opt1": false, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 2, 
					"name": "CNN_CCS", 
					"nameNoOptions": "CNN", 
					"short_description": "CNN1 + CCS", 
					"ultra_short_description": "CNN+C", 
					"desc": "long and eloquent scenario description", 
					"opt0": true, 
					"opt1": false, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 3, 
					"name": "CNN_copy_CCS", 
					"nameNoOptions": "CNN_copy", 
					"short_description": "CNN1 + CCS", 
					"ultra_short_description": "CNN+C", 
					"desc": "long and eloquent scenario description", 
					"opt0": true, 
					"opt1": false, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 4, 
					"name": "CNN_CCS_Bio", 
					"nameNoOptions": "CNN", 
					"short_description": "CNN + CCS + Bio", 
					"ultra_short_description": "CNN+C+B", 
					"desc": "long and eloquent scenario description", 
					"opt0": true, 
					"opt1": true, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 5, 
					"name": "CNN_copy_CCS_Bio", 
					"nameNoOptions": "CNN_copy", 
					"short_description": "CNN + CCS + Bio", 
					"ultra_short_description": "CNN+C+B", 
					"desc": "long and eloquent scenario description", 
					"opt0": true, 
					"opt1": true, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 6, 
					"name": "CNN_Bio", 
					"nameNoOptions": "CNN", 
					"short_description": "CNN + Bio", 
					"ultra_short_description": "CNN+B", 
					"desc": "long and eloquent scenario description", 
					"opt0": false, 
					"opt1": true, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 7, 
					"name": "CNN_copy_Bio", 
					"nameNoOptions": "CNN_copy", 
					"short_description": "CNN + Bio", 
					"ultra_short_description": "CNN+B", 
					"desc": "long and eloquent scenario description", 
					"opt0": false, 
					"opt1": true, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 8, 
					"name": "CNNold", 
					"nameNoOptions": "CNNold", 
					"short_description": "CNNold", 
					"ultra_short_description": "CNNo", 
					"desc": "long and eloquent scenario description", 
					"opt0": false, 
					"opt1": false, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 9, 
					"name": "CNN_copy_copy_Bio", 
					"nameNoOptions": "CNN_copy_copy", 
					"short_description": "CNN + Bio", 
					"ultra_short_description": "CNN+B", 
					"desc": "long and eloquent scenario description", 
					"opt0": false, 
					"opt1": true, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 10, 
					"name": "CNNold_copy", 
					"nameNoOptions": "CNNold_copy", 
					"short_description": "CNNold", 
					"ultra_short_description": "CNNo", 
					"desc": "long and eloquent scenario description", 
					"opt0": false, 
					"opt1": false, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 11, 
					"name": "CNNold_CCS", 
					"nameNoOptions": "CNNold", 
					"short_description": "CNNold + CCS", 
					"ultra_short_description": "CNNo+C", 
					"desc": "long and eloquent scenario description", 
					"opt0": true, 
					"opt1": false, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 12, 
					"name": "CNNold_copy_CCS", 
					"nameNoOptions": "CNNold_copy", 
					"short_description": "CNNold + CCS", 
					"ultra_short_description": "CNNo+C", 
					"desc": "long and eloquent scenario description", 
					"opt0": true, 
					"opt1": false, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 13, 
					"name": "CNNold_CCS_Bio", 
					"nameNoOptions": "CNNold", 
					"short_description": "CNNold + CCS + Bio", 
					"ultra_short_description": "CNNo+C+B", 
					"desc": "long and eloquent scenario description", 
					"opt0": true, 
					"opt1": true, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 14, 
					"name": "CNNold_copy_copy_CCS", 
					"nameNoOptions": "CNNold_copy_copy", 
					"short_description": "CNNold + CCS", 
					"ultra_short_description": "CNNo+C", 
					"desc": "long and eloquent scenario description", 
					"opt0": true, 
					"opt1": false, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 15, 
					"name": "CNNold_copy_CCS_Bio", 
					"nameNoOptions": "CNNold_copy", 
					"short_description": "CNNold + CCS + Bio", 
					"ultra_short_description": "CNNo+C+B", 
					"desc": "long and eloquent scenario description", 
					"opt0": true, 
					"opt1": true, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 16, 
					"name": "CNNold_copy_copy_CCS_Bio", 
					"nameNoOptions": "CNNold_copy_copy", 
					"short_description": "CNNold + CCS + Bio", 
					"ultra_short_description": "CNNo+C+B", 
					"desc": "long and eloquent scenario description", 
					"opt0": true, 
					"opt1": true, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 17, 
					"name": "CNNold_Bio", 
					"nameNoOptions": "CNNold", 
					"short_description": "CNNold + Bio", 
					"ultra_short_description": "CNNo+B", 
					"desc": "long and eloquent scenario description", 
					"opt0": false, 
					"opt1": true, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 18, 
					"name": "CNNold_copy_Bio", 
					"nameNoOptions": "CNNold_copy", 
					"short_description": "CNNold + Bio", 
					"ultra_short_description": "CNNo+B", 
					"desc": "long and eloquent scenario description", 
					"opt0": false, 
					"opt1": true, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 19, 
					"name": "CNNold_copy_copy_Bio", 
					"nameNoOptions": "CNNold_copy_copy", 
					"short_description": "CNNold + Bio", 
					"ultra_short_description": "CNNo+B", 
					"desc": "long and eloquent scenario description", 
					"opt0": false, 
					"opt1": true, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 20, 
					"name": "CNN_copy_copy", 
					"nameNoOptions": "CNN_copy", 
					"short_description": "CNN", 
					"ultra_short_description": "CNN", 
					"desc": "long and eloquent scenario description", 
					"opt0": false, 
					"opt1": false, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 21, 
					"name": "CNN_copy_copy_CCS", 
					"nameNoOptions": "CNN_copy", 
					"short_description": "CNN1 + CCS", 
					"ultra_short_description": "CNN+C", 
					"desc": "long and eloquent scenario description", 
					"opt0": true, 
					"opt1": false, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 22, 
					"name": "CNN_copy_copy_CCS_Bio", 
					"nameNoOptions": "CNN_copy", 
					"short_description": "CNN + CCS + Bio", 
					"ultra_short_description": "CNN+C+B", 
					"desc": "long and eloquent scenario description", 
					"opt0": true, 
					"opt1": true, 
					"opt2": false, 
					"opt3": false, 
				}, 
				{ 
					"id": 23, 
					"name": "CNNold_copy_copy", 
					"nameNoOptions": "CNNold_copy", 
					"short_description": "CNNold", 
					"ultra_short_description": "CNNo", 
					"desc": "long and eloquent scenario description", 
					"opt0": false, 
					"opt1": false, 
					"opt2": false, 
					"opt3": false, 
				}, 
			], 
			optionsAvailable: { 
				"CNN": { 
					"opt0": true,
					"opt1": true,
					"opt2": false,
					"opt3": false,
					}, 
				"CNN_copy": { 
					"opt0": true,
					"opt1": true,
					"opt2": false,
					"opt3": false,
					}, 
				"CNNold": { 
					"opt0": false,
					"opt1": true,
					"opt2": false,
					"opt3": false,
					}, 
				"CNNold_copy": { 
					"opt0": false,
					"opt1": true,
					"opt2": false,
					"opt3": false,
					}, 
			} 
		} 
	};